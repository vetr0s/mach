...
**mach.h** repo — and this repo consumes a vendored copy at
`third_party/mach/mach.h`. Engine internals (the batch renderer, the GL
loader, arenas, input, the Clay binding) are documented there; this document
covers the game and how it sits on the engine.
...
- **Quality is coherence and taste, not a feature count.** Say no to things the
  game doesn't need.
...
```
third_party/mach/mach.h             # the engine, vendored (its own repo has the docs)
...
Updating the engine means pasting a new mach.h release into `third_party/mach/`
(and fixing whatever its changelog says changed). Local experiments against the
vendored copy are fine — that's how engine changes get proven before they move
upstream — but the engine's source of truth is its own repo.
...
- **The frame arena** (`Mach.frame_arena`): per-frame scratch, reset at every
  ...
  render depth-sort buffers come from here, sized to what's actually alive.
...
- **A generic component-system ECS.** Entities are fat structs. You loop over an
  array.
- **Engine features for their own sake.** The engine grows in its own repo, on
  its own judgment; this game asks for things by need, not speculation.
...
8. **SDL3 → RGFW + own GL batch renderer** — done. Every dependency became a
...
11. **Repo split** — done. The engine (fully namespaced, with RGFW/Clay/stb_image
   ...
   game. This repo vendors a copy in `third_party/mach/` like any other
