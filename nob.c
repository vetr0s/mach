...
#define GEN_DIR BUILD_DIR "/generated"
#define SPRITES_HEADER GEN_DIR "/assets_sprites.h"
#define SPRITES_DIR "assets/sprites"

static int cstr_cmp(const void *a, const void *b) {
    return strcmp(*(const char *const *)a, *(const char *const *)b);
}

// Bake assets/sprites/*.png into a generated C header, so the shipped binary
// carries its art instead of hunting for a directory at runtime. The one static
// self-contained executable per platform is the point; a sprite loaded from a
// path would quietly break it. The game decodes these bytes at startup through
// mach_image_load_from_memory.
//
...
static bool embed_sprites(void) {
    ...
    // Keep only .png, and sort so the generated header is byte-stable across
    // filesystems that hand back directory entries in different orders.
    Nob_File_Paths pngs = {0};
    for (size_t i = 0; i < files.count; i++) {
        const char *name = files.items[i];
        size_t n = strlen(name);
        if (n > 4 && strcmp(name + n - 4, ".png") == 0) nob_da_append(&pngs, name);
    }
    qsort(pngs.items, pngs.count, sizeof(*pngs.items), cstr_cmp);

    Nob_String_Builder out = {0};
    nob_sb_append_cstr(&out,
        "// GENERATED by nob.c from " SPRITES_DIR "/*.png. Do not edit.\n"
        "//\n"
        "// Each sprite's encoded PNG bytes, baked in so the binary ships alone.\n"
        "#ifndef ASSETS_SPRITES_H\n"
        "#define ASSETS_SPRITES_H\n"
        "\n"
        "typedef struct {\n"
        "    const char *name;          // file stem: \"ore.png\" -> \"ore\"\n"
        "    const unsigned char *data; // encoded PNG, not raw pixels\n"
        "    int size;\n"
        "} Embedded_Sprite;\n"
        "\n");
    ...
    for (size_t i = 0; i < pngs.count; i++) {
        file.count = 0;
        const char *path = nob_temp_sprintf(SPRITES_DIR "/%s", pngs.items[i]);
        if (!nob_read_entire_file(path, &file)) return false;

        nob_sb_append_cstr(&out, nob_temp_sprintf("static const unsigned char asset_%zu_png[] = {", i));
        for (size_t b = 0; b < file.count; b++) {
            if (b % 16 == 0) nob_sb_append_cstr(&out, "\n    ");
            nob_sb_append_cstr(&out, nob_temp_sprintf("0x%02x,", (unsigned char)file.items[b]));
        }
        nob_sb_append_cstr(&out, "\n};\n\n");
    }

    nob_sb_append_cstr(&out, "static const Embedded_Sprite EMBEDDED_SPRITES[] = {\n");
    if (pngs.count == 0) {
        // A zero-length array is not valid C99. Count is what the game trusts.
        nob_sb_append_cstr(&out, "    {0, 0, 0},\n");
    }
    for (size_t i = 0; i < pngs.count; i++) {
        const char *name = pngs.items[i];
        size_t stem = strlen(name) - 4; // drop ".png"
        nob_sb_append_cstr(&out, nob_temp_sprintf("    {\"%.*s\", asset_%zu_png, (int)sizeof asset_%zu_png},\n",
                                                  (int)stem, name, i, i));
    }
    nob_sb_append_cstr(&out, "};\n\n");
    nob_sb_append_cstr(&out, nob_temp_sprintf("#define EMBEDDED_SPRITE_COUNT %zu\n\n", pngs.count));
    nob_sb_append_cstr(&out, "#endif // ASSETS_SPRITES_H\n");

    // Rewrite only on change: touching the header every build would rebuild the
    // game every build, and nob's rebuild check is timestamp-based.
    Nob_String_Builder old = {0};
    bool same = nob_file_exists(SPRITES_HEADER) && nob_read_entire_file(SPRITES_HEADER, &old) &&
                old.count == out.count && memcmp(old.items, out.items, out.count) == 0;
    if (!same) {
        nob_log(NOB_INFO, "Embedding %zu sprite(s) -> %s", pngs.count, SPRITES_HEADER);
        if (!nob_write_entire_file(SPRITES_HEADER, out.items, out.count)) return false;
    }
    return true;
}
...
static bool build_monolith(bool release) {
    ...
    Nob_Cmd cmd = {0};
#if defined(_WIN32)
    nob_cmd_append(&cmd, CC, "/std:c11", "/W4", "/Isrc", "/I" GEN_DIR);
    ...
    cmd_append_libs(&cmd);
#else
    nob_cmd_append(&cmd, CC, "-std=c99", "-Wall", "-Wextra", "-Isrc", "-I" GEN_DIR);
...
static bool build_game_lib(void) {
    ...
    Nob_Cmd cmd = {0};
    nob_cmd_append(&cmd, CC, "-std=c99", "-Wall", "-Wextra", "-Isrc", "-I" GEN_DIR, "-g", "-O0");
    #if defined(__APPLE__)
...
static bool build_hot(void) {
    ...
    nob_log(NOB_INFO, "Watching src/ and " SPRITES_DIR " for changes (Ctrl-C or close the game to stop)");
    ...
    for (;;) {
        ...
        // Re-bake the sprite header so saving a .png in Aseprite reloads the art
        // the same way saving a .c reloads the code. It rewrites the header only
        // when the bytes changed, so an untouched asset dir is a no-op here.
        if (!embed_sprites()) {
            nob_log(NOB_WARNING, "Sprite embed failed. Still watching.");
        }

        // Rebuild the library when any source is newer than it. After a build the
        // library is the newest file, so this settles until the next edit. The
        // generated sprite header is a source too: new art, new library.
        Nob_File_Paths sources = {0};
        if (!collect_sources("src", &sources)) { nob_da_free(sources); break; }
        nob_da_append(&sources, SPRITES_HEADER);
...
int main(int argc, char **argv) {
    ...
    // Every build path compiles the generated sprite header, so bake it first.
    if (!embed_sprites()) return 1;
