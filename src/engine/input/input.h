...
// instead of draining SDL events itself.
//
...
// of the next frame. Read fields directly: in->key_pressed[SDL_SCANCODE_1].
...
typedef struct {
    // Keyboard, indexed by SDL_Scancode. key_pressed excludes OS key repeats.
    u8 key_down[SDL_SCANCODE_COUNT];
    u8 key_pressed[SDL_SCANCODE_COUNT];

    // Mouse, in render coordinates (window points). wheel is this frame's scroll,
    // positive away from the user, already normalized for flipped/"natural" mode.
    Vec2 mouse;
    Vec2 mouse_delta;
    f32  wheel;
...
// Fold one SDL event into the snapshot. Events the snapshot doesn't model are ignored.
