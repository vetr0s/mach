...
static void draw_tile(Renderer *r, const Camera2D *cam, f32 gx, f32 gy, Color color) {
    Vec2 n, e, s, w;
...
static void draw_tile_edges(Renderer *r, const Camera2D *cam, f32 gx, f32 gy, Color color) {
    Vec2 n, e, s, w;
    tile_corners(r, cam, gx, gy, 0.0f, &n, &e, &s, &w);
    Vec2 pts[4] = {n, e, s, w};
...
static void draw_block(Renderer *r, const Camera2D *cam, f32 gx, f32 gy, f32 h, Color base) {
    ...
    Color line = color_shade(base, 0.30f);
...
static void draw_belt_tread(Renderer *r, const Camera2D *cam, f32 ax, f32 ay,
                            f32 bx, f32 by, f32 half_thick, Color color) {
    f32 sw = (f32)r->width, sh = (f32)r->height;
...
static void draw_belt_surface(Renderer *r, const Camera2D *cam, f32 gx, f32 gy,
                              Direction dir, f32 phase, Color color) {
    f32 dx = (f32)DIR_DX[dir], dy = (f32)DIR_DY[dir];
    f32 px = -dy, py = dx;         // perpendicular in grid space
...
static Color item_color(const Item *it) {
