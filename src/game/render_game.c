...
static void draw_miner(Gpu_Renderer *gpu, Game_Render *gr, const Entity_Miner *m) {
    Vec3 pos = {(f32)m->grid_x, 0.4f, (f32)m->grid_y};
    Mat4 model = model_ts(pos, (Vec3){0.8f, 0.8f, 0.8f});
    gpu_draw_mesh(gpu, &gr->cube, model, (Vec4){0.55f, 0.35f, 0.17f, 1.0f});
}

static void draw_storage(Gpu_Renderer *gpu, Game_Render *gr, const Entity_Storage *s) {
    Vec3 pos = {(f32)s->grid_x, 0.4f, (f32)s->grid_y};
    Mat4 model = model_ts(pos, (Vec3){0.8f, 0.8f, 0.8f});
    gpu_draw_mesh(gpu, &gr->cube, model, (Vec4){0.6f, 0.6f, 0.62f, 1.0f});
...
void game_render_draw(Gpu_Renderer *gpu, Game_Render *gr, const Game_State *game) {
    ...
    gpu_draw_mesh(gpu, &gr->plane, ground, (Vec4){0.16f, 0.20f, 0.24f, 1.0f});
    ...
    if (w) {
        for (i32 i = 0; i < w->entity_count; i++) {
            ...
            if (e->type == ENTITY_MINER)   draw_miner(gpu, gr, &e->data.miner);
            else if (e->type == ENTITY_STORAGE) draw_storage(gpu, gr, &e->data.storage);
        }
    }
    ...
    if (game->hover_valid) {
        ...
        gpu_draw_mesh(gpu, &gr->cube, model, color);
    }
}
